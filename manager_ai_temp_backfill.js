...
// Menu and prep recipe links. Menus already store recipe ids and generated prep
// tasks already carry recipeId; this exposes those links in the working UI.
(function(){
  'use strict';
  if(typeof VIEWS==='undefined')return;

  const clean=s=>String(s||'').trim().toLowerCase().replace(/^prep\s*[—–-]\s*/i,'').replace(/\s+/g,' ');
  const recipeById=id=>(STATE.recipes||[]).find(r=>r.id===id)||null;
  const recipeForPrep=p=>{
    const direct=p&&p.recipeId&&recipeById(p.recipeId);
    if(direct)return direct;
    const n=clean(p&&p.item);
    return (STATE.recipes||[]).find(r=>clean(r.name)===n)||null;
  };
  const ingredientName=i=>{
    const s=i&&i.stockId&&typeof stockById==='function'?stockById(i.stockId):null;
    return (s&&s.item)||(i&&i.name)||'Ingredient';
  };
  const ingredientLine=i=>{
    let out=ingredientName(i);
    if(i&&i.qty!=null)out+=' — '+i.qty+(i.unit?' '+i.unit:'');
    if(i&&i.preparation)out+=' · '+i.preparation;
    return out;
  };

  function recipeCard(r,p){
    const box=el('div',{class:'card',style:'margin-top:10px;background:var(--bg2);border-color:var(--line)'});
    const head=el('div',{class:'card-head'});
    head.append(el('div',{},el('div',{class:'eyebrow'},'Prep recipe'),el('h3',{style:'margin:2px 0 0'},r.name)),el('div',{class:'spacer'}));
    ...
    box.append(head);
    if(p)box.append(el('div',{class:'muted',style:'font-size:12px;margin:-4px 0 10px'},p.item+' · '+p.done+' / '+p.par+' done'));
    ...
    left.append(el('div',{class:'eyebrow',style:'margin-bottom:6px'},'Ingredients'));
    if((r.ingredients||[]).length)(r.ingredients||[]).forEach(i=>left.append(el('div',{class:'docket',style:'padding:7px 9px;margin-bottom:5px'},el('div',{class:'dk-t',style:'font-size:12.5px'},ingredientLine(i)))));
    else left.append(el('div',{class:'muted',style:'font-size:12.5px'},'No ingredients entered yet.'));
    right.append(el('div',{class:'eyebrow',style:'margin-bottom:6px'},'Method'),el('div',{style:'white-space:pre-wrap;font-size:12.5px;line-height:1.55'},String(r.method||'No method entered yet.')));
    grid.append(left,right);box.append(grid);
    return box;
  }
  ...
  VIEWS.menus=function(v){
    ...
    [...v.querySelectorAll('.docket')].forEach(row=>{
      ...
      if(!title||row.dataset.recipeLinkAdded)return;
  ...
  VIEWS.prep=function(v){
    ...
    card.append(el('div',{class:'card-head'},el('h3',{},"Today's prep recipes"),el('div',{class:'spacer'}),el('span',{class:'chip mono'},linked.length+' linked')),
      el('div',{class:'muted',style:'font-size:12.5px;margin-top:-5px;margin-bottom:8px'},'The menu/prep recipe is shown here so you can work from the specification without leaving Prep.'));
    linked.forEach(x=>card.append(recipeCard(x.recipe,x.task)));
    ...
    if(unlinked.length){
      ...
      unlinked.forEach(p=>{
        ...
        pick.addEventListener('change',()=>{
          ...
          p.recipeId=pick.value;p.consumed=false;
