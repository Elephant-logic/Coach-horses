(function(){
  function boot(){
    ...
    if(window.__recipeQuantityAutofillV1) return;
    window.__recipeQuantityAutofillV1=true;
    ...
    async function calculateBaseRecipes(recipes){
      const payload=recipes.map(r=>({
        id:String(r.id),name:r.name,course:r.course||r.category||'Other',portions:Math.max(1,num(r.portions||r.yield)||10),method:r.method||'',ingredients:(r.ingredients||[]).map(i=>typeof i==='string'?{name:i}:{name:i.name,qty:i.qty||i.quantity,unit:i.unit})
      }));
    ...
    async function handleBuild(e){
      ...
      const needs=recipes.filter(recipeNeedsWork);
      if(!needs.length) return;
      e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
      busy=true;button.disabled=true;
      status('Working out real recipe quantities for '+needs.length+' dishes…');
      try{
        ...
        for(const item of generated){
          ...
          recipe.ingredients=item.ingredients.map(i=>({name:String(i.name||'Ingredient').trim(),qty:Math.max(0.01,num(i.qty)),unit:norm(i.unit)==='litres'?'l':norm(i.unit)==='litre'?'l':String(i.unit||'g').trim()})).filter(i=>i.name&&i.qty>0);
